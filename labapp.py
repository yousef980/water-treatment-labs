...
class EliteWaterSuite(tk.Tk):
    ...
    def import_laboratory_file(self):
        file_path = filedialog.askopenfilename(
            filetypes=[("All Lab Formats", "*.csv *.xlsx *.xls *.txt *.md *.docx"),
        ...
        try:
            if ext == '.csv':
                ...
                messagebox.showinfo("Success", f"Imported CSV data sheet successfully!\nLoaded {len(df)} data points.")
                return df
    ...
    def build_framework_layout(self):
        ...
        self.scroll_frame.bind("<Configure>", lambda e: canvas_sb.configure(scrollregion=canvas_sb.bbox("all")))
        ...
        for display_name, routing_target in links:
            btn = tk.Button(self.scroll_frame, text=display_name, bg=self.clr_sidebar, fg=self.clr_text,
                            font=("Segoe UI", 9), bd=0, activebackground="#12141c", activeforeground=self.clr_accent,
                            anchor="w", padx=10, pady=8, command=lambda t=routing_target: self.navigate_to(t))
            btn.pack(fill="x", pady=1)
            
        footer_pane = tk.Frame(self.sidebar, bg=self.clr_sidebar, height=110)
        footer_pane.pack(side="bottom", fill="x", padx=15, pady=10)
        
        back_btn = tk.Button(footer_pane, text="<- Universal Track-Back", bg="#161924", fg=self.clr_text,
                             font=("Segoe UI", 9, "bold"), bd=0, activebackground=self.clr_accent, pady=8,
                             command=self.execute_track_back)
        back_btn.pack(fill="x", pady=2)

        home_btn = tk.Button(footer_pane, text="[MainMenu] Universal Home", bg=self.clr_accent, fg=self.clr_sidebar,
                             font=("Segoe UI", 9, "bold"), bd=0, activebackground=self.clr_success, pady=8,
    ...
    def execute_track_back(self):
        if len(self.navigation_history) <= 1:
            messagebox.showinfo("Navigation", "Base layer active."); return
    ...
    def render_view_by_target(self, view_name):
        ...
        views = {
            ...
            "IronManganese": lambda: self.draw_generic_placeholder("04. Iron & Manganese Groundwater Removal"),
            "NitrateAdsorption": self.draw_nitrate_adsorption_view,
            "LimeSoda": lambda: self.draw_generic_placeholder("06. Chemical Precipitation Softening (Lime-Soda)"),
            "GramStaining": lambda: self.draw_generic_placeholder("07. Microbiology Identification (Gram Staining)"),
            "SurfaceHygiene": lambda: self.draw_generic_placeholder("08. Surface Hygiene Monitor & Swab Analysis"),
            "WaterContamination": lambda: self.draw_generic_placeholder("09. Biological Water Quality Coliform Tracking")
        }
    ...
    def draw_dashboard_view(self):
        title = tk.Label(self.current_frame, text="Laboratory Automation Ecosystem Core", bg=self.clr_bg, fg=self.clr_text, font=("Segoe UI", 16, "bold"))
        title.pack(anchor="w", pady=(5, 2))
        
        desc = tk.Label(self.current_frame, text="Master of Environmental Engineering Laboratory Suite Dashboard Console.", bg=self.clr_bg, fg=self.clr_muted, font=("Segoe UI", 9))
        ...
        grid_frame.bind("<Configure>", lambda e: canvas_db.configure(scrollregion=canvas_db.bbox("all")))
...
modules = [
("01. Coagulation Processing Engine", "Jar Test curves with automated data handling vectors.", "JarTest"),
("02. Breakpoint Disinfection Model", "Chemical dosing trajectories for water sanitation processing.", "Chlorination"),
("03. Gravimetric Analytics Matrix", "Total Suspended Solids filter assay tracking workflows.", "SuspendedSolids"),
("04. Iron & Manganese Removal", "Oxidation and precipitation extraction processing from aquifers.", "IronManganese"),
("05. Nitrate Adsorption Kinetics", "Adsorption isothermal data modeling matrices.", "NitrateAdsorption"),
("06. Lime-Soda Precipitation Softening", "Automated carbonate hardness calculation matrices.", "LimeSoda"),
("07. Gram Staining Microbiology", "Microscopic morphology data metrics documentation modules.", "GramStaining"),
("08. Surface Hygiene Assessment", "Sanitation monitoring swabbing analytics.", "SurfaceHygiene"),
("09. Biological Water Contamination", "Coliform colony calculations and tracking pipelines.", "WaterContamination"),
]
for name, details, route in modules:
card = tk.Frame(grid_frame, bg=self.clr_card, bd=0, padx=15, pady=12)
card.pack(fill="x", pady=4)
lbl_name = tk.Label(card, text=name, bg=self.clr_card, fg=self.clr_accent, font=("Segoe UI", 11, "bold"))
lbl_name.pack(anchor="w")
lbl_det = tk.Label(card, text=details, bg=self.clr_card, fg=self.clr_text, font=("Segoe UI", 9), justify="left")
lbl_det.pack(anchor="w", pady=2)
launch_btn = tk.Button(card, text="Open Workspace ->", bg="#242938", fg=self.clr_accent, font=("Segoe UI", 8, "bold"), bd=0, padx=10, pady=4, command=lambda r=route: self.navigate_to(r))
...
upload_btn = tk.Button(self.ctrl_panel, text="[Open] Auto-Ingest Lab File", bg="#242938", fg=self.clr_accent, font=("Segoe UI", 9, "bold"), bd=0, pady=8, command=self.handle_jar_file_import)
upload_btn.pack(fill="x", pady=(5, 10))
tk.Label(self.ctrl_panel, text="Manual Dosing Entries (mg/L):", bg=self.clr_card, fg=self.clr_text).pack(anchor="w")
self.entry_x = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.entry_x.pack(fill="x", pady=(2, 8)); self.entry_x.insert(0, "10, 20, 30, 40, 50, 60")
tk.Label(self.ctrl_panel, text="Observed Residual Turbidity (NTU):", bg=self.ctrl_card, fg=self.clr_text).pack(anchor="w")
self.entry_y = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.entry_y.pack(fill="x", pady=(2, 15)); self.entry_y.insert(0, "14.2, 8.5, 1.8, 4.3, 9.1, 15.4")
...
messagebox.showerror("Error", "Invalid numeric sequencing layout."); return
...
ax.set_title("Turbidity Optimization Curve", color=self.clr_text, fontsize=10, fontweight="bold")
ax.tick_params(colors=self.clr_text, labelsize=8)
ax.grid(True, color="#2d3142", linestyle=":")
ax.text(opt["Dose"] + 1, opt["Turbidity"] + 2, f"Optimal: {opt['Dose']} mg/L", color=self.clr_success, fontweight="bold", fontsize=9)
...
upload_btn = tk.Button(self.ctrl_panel, text="[Open] Auto-Ingest Lab File", bg="#242938", fg=self.clr_accent, font=("Segoe UI", 9, "bold"), bd=0, pady=8, command=self.handle_chlorine_file_import)
upload_btn.pack(fill="x", pady=(5, 10))
...
self.cl_dose.pack(fill="x", pady=(2, 8)); self.cl_dose.insert(0, "0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0")
...
self.cl_res.pack(fill="x", pady=(2, 15)); self.cl_res.insert(0, "0.4, 0.8, 1.1, 0.6, 0.2, 0.5, 1.0, 1.5")
...
df = self.import_laboratory_file()
if df is not None and len(df) >= 2:
self.cl_dose.delete(0, tk.END); self.cl_res.delete(0, tk.END)
...
messagebox.showerror("Error", "Check data entry spacing."); return
self.clear_display_canvas()
fig, ax = plt.subplots(figsize=(5, 3.5), facecolor=self.clr_card)
ax.set_facecolor(self.clr_card)
...
ax.set_title("Breakpoint Chemistry Profile", color=self.clr_text, fontsize=10, fontweight="bold")
ax.tick_params(colors=self.clr_text, labelsize=8)
ax.grid(True, color="#2d3142", linestyle=":")
...
self.ss_vol.pack(fill="x", pady=(2, 8)); self.ss_vol.insert(0, "100")
tk.Label(self.ctrl_panel, text="Clean Filter Mass (g):", bg=self.clr_card, fg=self.clr_text).pack(anchor="w")
self.ss_w1 = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.ss_w1.pack(fill="x", pady=(2, 8)); self.ss_w1.insert(0, "1.2435")
tk.Label(self.ctrl_panel, text="Dried Residue Filter Weight (g):", bg=self.clr_card, fg=self.clr_text).pack(anchor="w")
self.ss_w2 = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.ss_w2.pack(fill="x", pady=(2, 15)); self.ss_w2.insert(0, "1.2488")
...
messagebox.showerror("Error", "Review entries."); return
self.clear_display_canvas()
out_lbl = tk.Label(self.display_panel, text=f"Total Suspended Solids (TSS):\n\n{tss_mg_l:.2f} mg/L", bg=self.clr_card, fg=self.clr_success, font=("Consolas", 16, "bold"))
out_lbl.pack(expand=True)
# -------------------------------------------------------------
# LAB MODULE 05: NITRATE ADSORPTION KINETICS
# -------------------------------------------------------------
def draw_nitrate_adsorption_view(self):
self.setup_module_base_layout("05. Nitrate Adsorption Kinetics Modeling")
upload_btn = tk.Button(self.ctrl_panel, text="[Open] Auto-Ingest Adsorption File", bg="#242938", fg=self.clr_accent, font=("Segoe UI", 9, "bold"), bd=0, pady=8, command=self.handle_adsorption_file_import)
upload_btn.pack(fill="x", pady=(5, 10))
tk.Label(self.ctrl_panel, text="Equilibrium Conc. Ce (mg/L):", bg=self.clr_card, fg=self.clr_text).pack(anchor="w")
self.ads_ce = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.ads_ce.pack(fill="x", pady=(2, 8)); self.ads_ce.insert(0, "1.2, 2.5, 4.8, 8.2, 12.5")
tk.Label(self.ctrl_panel, text="Adsorbed Capacity qe (mg/g):", bg=self.clr_card, fg=self.clr_text).pack(anchor="w")
self.ads_qe = tk.Entry(self.ctrl_panel, bg=self.clr_bg, fg=self.clr_text, bd=1, insertbackground="white")
self.ads_qe.pack(fill="x", pady=(2, 15)); self.ads_qe.insert(0, "0.45, 0.78, 1.15, 1.42, 1.65")
run_btn = tk.Button(self.ctrl_panel, text="[Run] Fit Isotherm Models", bg=self.clr_success, fg=self.clr_sidebar, font=("Segoe UI", 10, "bold"), bd=0, pady=8, command=self.execute_adsorption_math)
run_btn.pack(fill="x")
def handle_adsorption_file_import(self):
df = self.import_laboratory_file()
if df is not None and len(df) >= 2:
self.ads_ce.delete(0, tk.END); self.ads_qe.delete(0, tk.END)
...
summary_text = f"Langmuir: qmax = {qmax:.2f} mg/g, b = {b_constant:.3f} L/mg | Freundlich: Kf = {Kf:.2f}, n = {n_constant:.2f}"
summary_lbl = tk.Label(self.display_panel, text=summary_text, bg=self.clr_card, fg=self.clr_accent, font=("Segoe UI", 9, "bold"))
...
lbl = tk.Label(self.display_panel, text="[Active Ingest Node]\nReady to receive analytical variables from laboratory equipment logsheets.", bg=self.clr_card, fg=self.clr_muted, font=("Segoe UI", 10, "italic"))
